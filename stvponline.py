...
st.markdown("""
    ...
    /* Títulos principales: Azul Navy Profundo para alto contraste */
    .section-title {
    ...
    /* Texto general: Gris Carbón (más moderno que el negro) */
    ...
    /* Contenedores Blancos con bordes suaves */
    .white-container {
    ...
    /* Ajuste para tablas */
    .stTable td { color: #2C3E50 !important; }
    </style>
...
# --- 4. CONEXIÓN G-SHEETS (Mantengo tu lógica original) ---
...
with c_title:
    st.markdown("<h1 style='color: #1A237E; margin-bottom:0; font-size: 3rem;'>STVP - Sindicato Vigilancia</h1>", unsafe_allow_html=True)
    st.markdown("<h4 style='color: #475569;'>Gestión, Turismo y Beneficios para el trabajador</h4>", unsafe_allow_html=True)
...
with ct1: 
    st.markdown("<h3 style='color: #1E293B;'>Temporada Verano 2026</h3>", unsafe_allow_html=True)
with ct2:
...
# Usando use_container_width para mejor adaptabilidad
...
with col_camp2:
    ...
    * **Alojamiento diario:** $10.000
    * **Parcela carpa:** $8.000
    """)
    st.info("📍 Ubicación: Ruta 8 km. 76, Exaltación de la Cruz")
st.markdown('</div>', unsafe_allow_html=True)
...
with col_lyf1:
    st.markdown("<h4 style='color: #1E293B;'>Convenio de Turismo Social - Temporada de Verano</h4>", unsafe_allow_html=True)
    st.write("Contamos con plazas exclusivas en San Bernardo, Mar del Plata y Villa Giardino.")
with col_lyf2:
    folleto_drive = "https://drive.google.com/file/d/1jhcYwAVWGr9ZSHIiAgaPyU_j4eJho_yx/view?usp=drive_link"
    st.markdown(f'<a href="{folleto_drive}" target="_blank" class="download-btn">📥 VER PDF COMPLETO</a>', unsafe_allow_html=True)
...
with st.form("afi_form", clear_on_submit=True):
    st.markdown("<p style='text-align:center;'>Complete el formulario y un asesor se pondrá en contacto.</p>", unsafe_allow_html=True)
    f1, f2 = st.columns(2)
    ...
    submit = st.form_submit_button("ENVIAR DATOS AHORA")
    if submit:
